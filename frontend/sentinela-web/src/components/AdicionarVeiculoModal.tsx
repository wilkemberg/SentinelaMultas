...
import { apiVeiculos } from "@/lib/api";
import { X, ShieldCheck, PlusCircle, AlertCircle, Loader2 } from "lucide-react";
...
export default function AdicionarVeiculoModal({ onClose, onSuccess }: Props) {
  const [form, setForm] = useState({ placa: "", renavam: "", uf: "RJ" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErro("");
    try {
      const v = await apiVeiculos.criar(form.placa, form.renavam.replace(/\D/g, ""), form.uf);
        ...
        {etapa === "form" ? (
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-nevoa mt-1">Varredura diária às 07:00 (D-1)</p>
                </div>
              </div>
              <button
                onClick={onClose}
          ...
          <div className="text-center py-6 flex flex-col items-center">
            ...
            <div className="mt-6 p-4 rounded-xl bg-asfalto w-full text-left">
              <p className="text-xs text-nevoa leading-relaxed">
                Varredura diária programada para 07:00. Bases consultadas: DETRAN-RJ, CET-RIO e PRF.
